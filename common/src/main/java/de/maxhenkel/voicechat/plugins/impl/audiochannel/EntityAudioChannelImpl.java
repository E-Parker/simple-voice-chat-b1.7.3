...
public class EntityAudioChannelImpl extends AudioChannelImpl implements EntityAudioChannel {
    ...
    private void broadcast(PlayerSoundPacket packet) {
        if (entity.getEntity() instanceof net.minecraft.entity.Entity) {
            ...
            server.broadcast(ServerWorldUtils.getPlayersInRange(entity.world, Vec3d.createCached(entity.x, entity.y, entity.z), server.getBroadcastRange(distance), filter == null ? player -> true : player -> filter.test(new ServerPlayerImpl(player))), packet, null, null, null, SoundPacketEvent.SOURCE_PLUGIN);
        }
    }
